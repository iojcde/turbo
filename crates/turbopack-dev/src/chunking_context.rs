...
impl DevChunkingContextBuilder {
    ...
    pub fn build(self) -> Vc<Box<dyn ChunkingContext>> {
        Vc::upcast(DevChunkingContext::new(Value::new(self.context)))
    }
}
...
impl ChunkingContext for DevChunkingContext {
    ...
    async fn chunk_group(self: Vc<Self>, entry_chunk: Vc<Box<dyn Chunk>>) -> Result<Vc<Assets>> {
        ...
        let mut assets: Vec<Vc<Box<dyn Asset>>> = optimized_chunks
            .await?
            .iter()
